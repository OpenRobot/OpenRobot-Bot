...
class OpenRobotFormatter(prettify_exceptions.DefaultFormatter):
    def __init__(self, **kwargs):
        kwargs["theme"] = {
            "_ansi_enabled": True
            if (
                not os.environ.get("OPENROBOT-FORMATTER_NO_COLOR", "True").lower()
                == "true"
            )
...
class Error:
    ...
    async def create(self, **kwargs) -> ErrorResult:
        ...
        return ErrorResult(
            **dict(
                await self.bot.tb_pool.fetchrow(
    ...
    async def get(self, **kwargs) -> list[ErrorResult] | ErrorResult:
        if not kwargs:
            return [
                ErrorResult(dict(x))
                for x in await self.bot.tb_pool.fetch("SELECT * FROM tracebacks")
            ]
        ...
        db = await self.bot.tb_pool.fetch(
            f"SELECT * FROM traceback WHERE {s}", *[val for val in kwargs.values()]
        )
