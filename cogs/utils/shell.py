...
class RunShell:
    def __init__(
        self, command: list[str] | str, *, loop: asyncio.AbstractEventLoop = None
    ):
        ...
        self.process = subprocess.Popen(
            self.decoded_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    def result(self) -> tuple[str | None, str | None, int | None]:
        return (
            self.clean_line(self.stdout),
            self.clean_line(self.stderr),
            int(self.clean_line(self.close_code)),
        )
