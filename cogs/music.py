...
from cogs.utils import (
    ...
    TimeConverter,
    is_guild_owner,
    QueueNowPlayingPaginator,
    ViewMenuPages,
    ClassicPaginator,
    QueueHistoryPaginator,
    Filters,
)
...
class Music(Cog, emoji="🎵"):
    ...
    async def initiate_node(self):
        await self.slate.create_node(
            bot=self.bot,
            identifier="OpenRobot Obsidian - 1",
            **self.bot.config.OBSIDIAN_SLATE_CRIDENTIALS,
        )
    ...
    async def get_user_near_expire(self) -> typing.Tuple[int, dict]:
        while True:
            try:
                res = await self.bot.spotify_pool.fetchrow(
                    "SELECT * FROM spotify_auth ORDER BY expires_at ASC"
                )
            except:
                pass
            else:
    ...
    async def renew_once(self, user_id: int = None, **kwargs):
        if not user_id:
            ...
            while True:
                try:
                    res = await self.bot.spotify_pool.fetchrow(
                        "SELECT * FROM spotify_auth WHERE user_id = $1", user_id
                    )
                ...
                headers={
                    "Authorization": (
                        ...
                        + base64.urlsafe_b64encode(
                            f'{SPOTIFY_CRIENDTIALS["client_id"]}:{SPOTIFY_CRIENDTIALS["client_secret"]}'.encode()
                        )
                    ).decode(),
                    ...
                    try:
                        ...
                        """,
                            ...
                            (
                                ...
                                + datetime.timedelta(seconds=js["expires_in"])
                            ),
                            js["expires_in"],
                        )
                    except asyncpg.exceptions._base.InterfaceError:
    ...
    async def on_obsidian_track_start(
        self, player: Player, _: obsidian.TrackStart
    ) -> None:
    ...
    async def on_obsidian_track_exception(
        self, player: Player, _: obsidian.TrackException
    ) -> None:
        await player.handle_track_error()

    @commands.Cog.listener()
    async def on_obsidian_track_stuck(
        self, player: Player, _: obsidian.TrackStuck
    ) -> None:
        await player.handle_track_error()
    ...
    async def connect(
        ...
        *,
        ...
        | discord.StageChannel = commands.Option(
            None, description="The channel to join to."
        ),
    ):
        """
    ...
    async def disconnect(self, ctx: commands.Context):
        ...
        if not ctx.voice_client:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="I am not in any Voice Channels!"
                )
            )
        ...
        if ctx.author not in ctx.voice_client.channel.members:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color,
                    description=f"You are not in {ctx.voice_client.channel.mention}!",
                )
            )
        ...
        return await ctx.send(
            embed=discord.Embed(
                color=self.bot.color, description=f"Left {chan.mention}"
            )
        )
    ...
    async def play(self, ctx: commands.Context, *, query: str = None):
        ...
        if query is None and ctx.voice_client and ctx.voice_client.paused is True:
            ...
            raise commands.MissingRequiredArgument(
                inspect.Parameter(
                    "query", inspect.Parameter.KEYWORD_ONLY, annotation=str
                )
            )
        ...
        if ctx.voice_client is None or ctx.voice_client.is_connected() is False:
            if (
                command := ctx.bot.get_command("connect")
            ) is None or await command.can_run(ctx) is True:
        ...
        async with ctx.channel.typing():
            m = await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color,
                    description="<a:openrobot_searching_gif:899928367799885834> Searching...",
                )
            )
            ...
            if from_liked_songs:
                try:
                    while True:
                        try:
                            x = await self.bot.spotify_pool.fetchrow(
                                "SELECT * FROM spotify_auth WHERE user_id = $1",
                                ctx.author.id,
                            )
                        except asyncpg.exceptions._base.InterfaceError:
                            pass
                    ...
                    if not access_token:
                        return await ctx.send(
                            "Please Sign-In to OpenRobot Spotify using `or.spotify login`."
                        )
                    ...
                    if not urls:
                        return await ctx.send(
                            "If you don't have any Spotify Liked Songs, how can I load them?"
                        )
                except Exception as e:
                    if ctx.debug:
                        raise e
                    ...
                    return await ctx.send(
                        "Something wen't wrong in our back-end, and we aren't able to query your Spotify Liked Songs."
                    )
                ...
                for url in urls:
                    track = (
                        await ctx.voice_client.search(
                            url, source=slate.Source.YOUTUBE, ctx=ctx
                        )
                    ).tracks[0]
                    tracks.append(track)
                    ctx.voice_client.queue.put(
                        track, position=0 if (flags.now or flags.next) else None
                    )
                    ...
                    if not has_send:
                        ...
                        await ctx.reply(
                            embed=discord.Embed(
                                color=self.bot.color,
                                description=f"Added {len(urls)} tracks to the queue from your Spotify Liked Songs.",
                            )
                        )
                ...
                await ctx.voice_client.queue_search(
                    query,
                    ctx=ctx,
                    now=flags.now,
                    next=flags.next,
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if not ctx.voice_client:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="There are no tracks playing."
                )
            )
        ...
        if ctx.voice_client.paused is True:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="The track is already paused."
                )
            )
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.color, description="The player is now paused."
            )
        )
    ...
    async def stop(self, ctx: commands.Context):
        ...
        if ctx.voice_client.paused is True:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="This track is already paused."
                )
            )
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.color, description="The player is now stopped."
            )
        )
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if ctx.voice_client.paused is False:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="The track is already resumed."
                )
            )
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.color, description="The player is now resumed."
            )
        )
    ...
    async def volume(
        self,
        ctx: commands.Context,
        volume: int = commands.Option(description="The volume to be set to."),
    ):
        """
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.color,
                description=f"Volume has been changed from **{old_volume*100}%** to **{new_volume*100}%**.",
            )
        )
    ...
    async def seek(
        ...
        ctx: commands.Context,
        *,
        time: TimeConverter = commands.Option(
            description="Seeks to the time provided."
        ),
    ):
        """
        ...
        if 0 < milliseconds > ctx.voice_client.current.length:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color,
                    description=f"That is not a valid amount of time, please choose a time between `0s` and `{ctx.voice_client.current.length // 1000}s`.",
                )
            )
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.color,
                description=f"The player position is now at `{ctx.voice_client.position // 1000}s` (`{humanize.naturaldelta(datetime.timedelta(seconds=ctx.voice_client.position // 1000))}`)",
            )
        )

    @music.command(
        "fast-forward", aliases=["fast_forward", "fastforward", "ff", "forward", "fwd"]
    )
    async def fast_forward(
        ...
        *,
        time: TimeConverter = commands.Option(
            description="Fast-forwards to the time provided"
        ),
    ):
        """
    ...
    async def rewind(
        ...
        *,
        time: TimeConverter = commands.Option(
            description="Rewinds to the time provided."
        ),
    ):
        """
    ...
    async def loop(
        ...
        ctx: commands.Context,
        *,
        loop_type: typing.Literal["Current", "Queue", "None"] = commands.Option(
            description="The new loop type to be set."
        ),
    ):
        if not ctx.voice_client or not ctx.voice_client.is_playing():
            return await ctx.send(
    ...
    async def skip(
        ...
        *,
        amount: int = commands.Option(
            1, description="Number of tracks to skip. Defaults to 1."
        ),
    ):
        """
        ...
        **amount**: The amount of tracks to skip, only works if you are the bot owner, guild owner, or have the manage guild, manage message, manage channels, kick members, or ban members permissions.
        ...
        try:
            ...
            if ctx.author.id == ctx.voice_client.current.requester.id:
                ...
                return await ctx.send(
                    embed=discord.Embed(
                        color=self.bot.color, description="Skipped the current track."
                    )
                )
            ...
            await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="Removed your vote to skip."
                )
            )
    ...
    async def save(self, ctx: commands.Context):
        ...
        try:
            embed = discord.Embed(
                ...
                if not ctx.voice_client.is_stream()
                else "**Length:** LIVE"
            ...
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="I am unable to DM you."
                )
            )
    ...
    async def queue(
        self,
        ctx: commands.Context,
        action: typing.Literal["Detailed", "Show"] = commands.Option(
            description="Views detailed or shows normally."
        ),
    ):
        """
    ...
    async def queue_show(self, ctx: commands.Context):
        ...
        if ctx.voice_client.queue.is_empty():
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="The queue is empty."
                )
            )

        queue = ctx.voice_client.queue

        await ViewMenuPages(QueueNowPlayingPaginator(queue, queue, per_page=10)).start(
            ctx
        )
    ...
    async def queue_history_show(self, ctx: commands.Context):
        ...
        if not history:
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color, description="The queue history is empty."
                )
            )
    ...
    async def clear(self, ctx: commands.Context):
        ...
        await ctx.send(
            embed=discord.Embed(
                colour=self.bot.color, description="The queue has been cleared."
            )
        )
    ...
    async def shuffle(self, ctx: commands.Context):
        ...
        await ctx.send(
            embed=discord.Embed(
                colour=self.bot.color, description="The queue has been shuffled."
            )
        )
    ...
    async def reverse(self, ctx: commands.Context):
        ...
        await ctx.send(
            embed=discord.Embed(
                colour=self.bot.color, description="The queue has been reversed."
            )
        )
    ...
    async def sort(
        self,
        ctx: commands.Context,
        method: typing.Literal["Title", "Length", "Author"] = commands.Option(
            description="Sorts by Title/Length/Author."
        ),
        reverse: bool = commands.Option(
            False, description="Whether to reverse the sort."
        ),
    ):
        """
        Sorts the queue.
        ...
        **Usage:**
        ...
        if method == "Title":
            ctx.voice_client.queue._queue.sort(
                key=lambda track: track.title, reverse=reverse
            )
        elif method == "Author":
            ctx.voice_client.queue._queue.sort(
                key=lambda track: track.author, reverse=reverse
            )
        elif method == "Length":
            ctx.voice_client.queue._queue.sort(
                key=lambda track: track.length, reverse=reverse
            )
    ...
    async def remove(
        self,
        ctx: commands.Context,
        entry: int = commands.Option(
            description="The entry/index of the track to be removed."
        ),
    ):
        """
        ...
        **entry**: The position of the track you want to remove.
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.color,
                description=f"Removed **[{item.title}]({item.uri})** which was requested by **{item.author}** from the queue.",
            )
        )
    ...
    async def move(
        self,
        ctx: commands.Context,
        entry_1: int = commands.Option(description="The original track's entry/index."),
        entry_2: int = commands.Option(
            description="The new entry/index for the track to be moved to."
        ),
    ):
        """
        ...
        **entry_2**: The position of the track you want to move too.
        ...
        if (entry_1 <= 0 or entry_1 > len(ctx.voice_client.queue)) or (
            entry_2 <= 0 or entry_2 > len(ctx.voice_client.queue)
        ):
            return await ctx.send(
                embed=discord.Embed(
                    color=self.bot.color,
    ...
    async def _filter(
        self,
        ctx: commands.Context,
        filter: typing.Literal["8D", "Nightcore"] = commands.Option(
            description="The filter to enable/disable."
        ),
    ):
        """
        Filter commands.
        ...
        if Filters.ROTATION in ctx.voice_client.enabled_filters:
            ...
            await ctx.voice_client.set_filter(
                obsidian.Filter(
                    ...
                    rotation=obsidian.Rotation(rotation_hertz=0.5),
                )
            )
    ...
    async def nightcore(self, ctx: commands.Context):
        ...
        if Filters.NIGHTCORE in ctx.voice_client.enabled_filters:
            ...
            await ctx.voice_client.set_filter(
                obsidian.Filter(
                    ctx.voice_client.filter,
                    timescale=obsidian.Timescale(speed=1.12, pitch=1.12),
                )
            )
    ...
    async def node_stats(self, ctx: commands.Context):
        ...
        for node in self.bot.slate.nodes.values():
            ...
            if node._stats:
                embed.description += (
                    f"**Threads (running):** {node._stats.threads_running}\n"
    ...
    @music.command(
        name="voice-clients", aliases=["voice_clients", "voiceclients", "vcs"]
    )
    ...
    async def voiceclient(
        self,
        ctx: commands.Context,
        *,
        server: discord.Guild = commands.Option(
            None, description="The server to show voice client detais."
        ),
    ):
        """
        ...
        if player.current:
            ...
            embed.add_field(
                name="__Track info:__", value=f"**Track:** None\n", inline=False
            )
        ...
        embed.add_field(
            ...
            if not any([track.is_stream() for track in player.queue])
            else "**Total time:** Unable to determine."
        ...
        if not player.queue.is_empty():
            ...
            embed.add_field(
                ...
                value="\n".join(
                    [
                        f"**{index + 1}.** [{entry.title}]({entry.uri}) by **{entry.author}**"
                        for index, entry in enumerate(list(player.queue)[:5])
                    ]
                )
                + (
                    f"\n ... **5** of **{len(player.queue)}** total."
        ...
        embed.add_field(
            ...
            + (
                f"\n ... **5** of **{len(player.listeners)}** total."
                if len(player.listeners) > 5
                else ""
            ),
            inline=False,
        )
